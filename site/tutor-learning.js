(()=>{
  ...
  async function refreshSession(current){
    ...
    try{
      ...
      const next={access_token:d.access_token,refresh_token:d.refresh_token||current.refresh_token,expires_at:Math.floor(Date.now()/1000)+(Number(d.expires_in)||3600),user:d.user||current.user||null};
  ...
  async function remoteProgress(){
    ...
    let r=await make();
    if(r.status===401){const renewed=await refreshSession(auth.session);if(renewed){auth={token:renewed.access_token,userId:renewed.user?.id||auth.userId,session:renewed};r=await make()}}
    if(!r.ok)return null;
  ...
  async function syncLesson(lessonId,state,correct){
    ...
    const make=()=>fetch(`${runtime.supabase}/rest/v1/tutor_progress?on_conflict=user_id,lesson_id`,{method:'POST',headers:{'Content-Type':'application/json',apikey:runtime.key,Authorization:`Bearer ${auth.token}`,Prefer:'resolution=merge-duplicates,return=minimal'},body:JSON.stringify(body)});
    let r=await make();
    if(r.status===401){const renewed=await refreshSession(auth.session);if(renewed){auth={token:renewed.access_token,userId:renewed.user?.id||auth.userId,session:renewed};body.user_id=auth.userId;r=await make()}}
  ...
  async function hydrateRemote(state){
    const rows=await remoteProgress();if(!Array.isArray(rows))return false;
    state.remote=state.remote||{};state.completed=state.completed||{};state.attempts=state.attempts||{};
    for(const row of rows){state.remote[row.lesson_id]=row;state.attempts[row.lesson_id]=Math.max(Number(state.attempts[row.lesson_id])||0,Number(row.attempts)||0);if(row.status==='completed')state.completed[row.lesson_id]=true}
    saveState(state);return true;
  }

  async function addLearning(){
    if(q('#tutor-learning-path')) return;
    const slot=q('#tutor-slot'),lab=q('#tutor-ai-lab');if(!slot&&!lab)return;
    const s=document.createElement('section');s.id='tutor-learning-path';s.className='tutor-learning';s.innerHTML='<div class="shell"><div class="learning-shell"><div class="intel-empty">Loading Tutor foundations…</div></div></div>';
    ...
    try{
      const [r,rt]=await Promise.all([fetch(`${FEED}?t=${Date.now()}`,{cache:'no-store'}),loadRuntime()]);if(!r.ok)throw new Error('curriculum');
      runtime=rt;curriculum=await r.json();const state=loadState();state.completed=state.completed||{};state.answers=state.answers||{};state.attempts=state.attempts||{};await hydrateRemote(state);render(curriculum,state);
    }catch{const host=q('.learning-shell',s);if(host)host.innerHTML='<div class="intel-empty">Tutor foundations are temporarily unavailable.</div>'}
  }

  function render(data,state=loadState()){
    const host=q('#tutor-learning-path .learning-shell');if(!host)return;
    const lessons=(data.lessons||[]).sort((a,b)=>(a.order||0)-(b.order||0));state.completed=state.completed||{};state.answers=state.answers||{};state.attempts=state.attempts||{};
    const done=lessons.filter(x=>state.completed[x.id]).length,s=loadSession(),signedIn=!!(s?.access_token&&s?.user?.id&&runtime);
    host.innerHTML=`<div class="learning-head"><div><div class="eyebrow">VAST Tutor · structured learning</div><h2>Foundations that can be measured.</h2><p>${signedIn?'Signed-in progress is synced securely to your private Tutor profile.':'Progress stays on this device until you sign in through Vision AI.'}</p></div><div class="learning-progress"><b>${percent(done,lessons.length)}%</b><span>${done}/${lessons.length} lessons completed</span><div><i style="width:${percent(done,lessons.length)}%"></i></div></div></div><div class="lesson-grid">${lessons.map((x,i)=>lessonCard(x,i,state)).join('')}</div><div class="learning-foot"><span>${signedIn?'PRIVATE SYNC ON':'LOCAL FALLBACK'}</span><p>${signedIn?'Lesson status, score and attempt count are stored in the authenticated backend under row-level security. Expired access tokens refresh automatically; screenshots are not stored here.':'Clearing browser storage resets local-only progress. Sign in through Vision AI to enable private cross-device progress sync.'}</p><button type="button" id="learning-reset">Reset local progress</button></div>`;bind(lessons,state,data);
  }

  function lessonCard(x,i,state){
    const answered=state.answers[x.id],complete=!!state.completed[x.id],attempts=Number(state.attempts?.[x.id])||0;
  ...
  function bind(lessons,state,data){
    ...
    lessons.forEach(x=>{const card=q(`[data-lesson="${CSS.escape(x.id)}"]`);if(!card)return;card.querySelectorAll('[data-answer]').forEach(btn=>btn.addEventListener('click',async()=>{const answer=Number(btn.dataset.answer),correct=answer===Number(x.quiz?.correct_index);state.answers[x.id]=answer;state.attempts[x.id]=(Number(state.attempts[x.id])||0)+1;if(correct)state.completed[x.id]=true;saveState(state);await syncLesson(x.id,state,correct);render(data,state)}))});
  }
